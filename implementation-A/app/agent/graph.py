...
logger = setup_logger("parakeet.agent")

def build_parakeet_agent(schema_context: str) -> Any:
    """Compiles and returns the LangGraph ReAct StateMachine with structured loop logging."""
    ...
    def agent_node(state: AgentState):
        ...
        # Compute current ReAct iteration count
        ai_messages = [m for m in messages if isinstance(m, AIMessage)]
        iteration = len(ai_messages) + 1

        logger.info(f"🧠 [ReAct Loop - Iteration #{iteration}] Reasoning & action planning...")
        response = llm.invoke(messages)

        # Log LLM Reasoning / Thought
        if response.content:
            logger.info(f"💭 [ReAct Thought #{iteration}]: {response.content.strip()}")

        # Log Action / Tool Call Intent
        if hasattr(response, "tool_calls") and response.tool_calls:
            for call in response.tool_calls:
                logger.info(
                    f"🛠️ [ReAct Action #{iteration}]: Calling Tool '{call['name']}' with args: {call['args']}"
                )
        else:
